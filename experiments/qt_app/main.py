...
from experiments.qt_app.orchestrator_adapter import (
    analyze_image_languages,     # -> Set[str]
    process_image_with_types,    # -> Path
    IMG_EXTS
)
...
class AnalyzeWorker(QObject):
    ...
    def run(self):
        try:
            ...
            langs = analyze_image_languages(self.image_path)
            res = sorted(langs)
...
class ProcessWorker(QObject):
    ...
    failed = Signal(str)
    def __init__(self, image_path: str, lang_keys: Set[str]):
        super().__init__()
        self.image_path = image_path
        self.lang_keys = lang_keys
    def run(self):
        try:
            self.progress.emit(5, "Starting pipeline…")
            doc_dir = process_image_with_types(self.image_path, self.lang_keys)
            self.progress.emit(100, "Done")
            self.finished.emit(str(doc_dir))
        except Exception as e:
            import traceback
            self.failed.emit(f"{e}\n{traceback.format_exc()}")
...
class MainWindow(QMainWindow):
    ...
    def on_recognize_clicked(self):
        if not self.current_image:
            QMessageBox.information(self, "No file", "Select an image from the list."); return
        self.status_lbl.setText("Recognizing types…")
        self.log("=== Recognize Types ==="); self.log(f"File: {self.current_image}")

        self.worker_thread = QThread()
        worker = AnalyzeWorker(str(self.current_image))
    ...
    def on_process_clicked(self):
        if not self.current_image:
            QMessageBox.information(self, "No file", "Select an image first."); return
        selected = self._selected_lang_keys()
        if not selected:
            QMessageBox.information(self, "Choose types", "Pick at least one language box."); return
        ...
        self.worker_thread = QThread()
        worker = ProcessWorker(str(self.current_image), selected)
        worker.moveToThread(self.worker_thread)
        self.worker_thread.started.connect(worker.run)
